...
static int h3_post_config(apr_pool_t* p, apr_pool_t* plog, apr_pool_t* ptemp, server_rec* s)
{
    ...
    /* Register a placeholder QUIC listener in parent so MPM adds it to pollset.
     * Actual socket creation happens per-child in h3_child_init (with SO_REUSEPORT).
     * We use a dummy socket here just to get into ap_listeners. */
    ...
    /* Create and bind UDP socket in PARENT - children will inherit it */
    ...
    /* Get fd - DON'T use SO_REUSEPORT (causes random packet distribution breaking QUIC)
     * For now, only ONE child will handle QUIC. Configure Apache with:
    ...
    /* Create Apache listener record in PARENT so MPM includes it in pollset */
    ...
    ap_log_error(APLOG_MARK, APLOG_INFO, 0, s,
                 "mod_http3: Registered QUIC listener on port %d (children will attach OpenSSL)",
                 conf->host_port);
...
static void h3_child_init(apr_pool_t* pchild, server_rec* s)
{
    ...
    /* Find the h3 listener that parent created - we'll inherit its socket */
    for (lr = ap_listeners; lr; lr = lr->next) {
        if (lr->protocol && strcmp(lr->protocol, "h3") == 0) {
            /* Found it - get the UDP socket parent created and bound */
            udp_sock = lr->sd;
            apr_os_sock_get(&fd, udp_sock);

            ap_log_error(APLOG_MARK, APLOG_INFO, 0, s,
                         "h3_child_init: found inherited QUIC socket on port %d, attaching OpenSSL",
                         conf->host_port);
            break;
        }
    }
    ...
    if (!lr) {
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, s,
                     "h3_child_init: Could not find h3 listener created by parent!");
        return;
    }

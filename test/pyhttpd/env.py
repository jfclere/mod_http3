...
class HttpdTestSetup:
    ...
    def _add_aptest(self):
        ...
        aptest_dir = os.path.join(local_dir, 'mod_aptest')
...
class HttpdTestEnv:
    ...
    def run(self, args, stdout_list=False, intext=None, inbytes=None, debug_log=True, timeout=None):
        if debug_log:
        ...
        try:
            p = subprocess.run(args, stderr=subprocess.PIPE, stdout=subprocess.PIPE,
                               input=inbytes, timeout=timeout)
        except subprocess.TimeoutExpired:
            log.warning(f"subprocess.run timed out after {timeout}s: {args}")
            return ExecResult(args=args, exit_code=124,
    ...
    def curl_raw(self, urls, timeout=10, options=None, insecure=False,
        ...
        args, headerfile = self.curl_complete_args(
            ...
            force_resolve=force_resolve)
        args += urls
        r = self.run(args, stdout_list=stdout_list, timeout=timeout)
        if r.exit_code == 0:
